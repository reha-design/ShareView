...
## 주의 사항
- **파일 전달 범위**: 호스트가 보낸 파일은 모든 시청자에게 가지만, **시청자가 보낸 파일은 호스트에게만** 전달됩니다(채팅과 달리 다른 시청자에게 중계하지 않음). 모달에는 현재 동작대로 안내했습니다. 중계 기능은 개발 예정으로 [CHANGELOG](../../CHANGELOG.md)에 기록했고, 이와 다르게 적혀 있던 [드래그 앤 드롭 보고서](patch_feature_drag_drop.md)도 실제 동작에 맞게 수정했습니다.
